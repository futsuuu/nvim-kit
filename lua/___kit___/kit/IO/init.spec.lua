...
describe('kit.IO', function()
  ...
  describe('.dirname', function()
    ...
    it('should handle over-level request', function()
      assert.are.equal(IO.dirname('/a/b', 3), '/a/b')
    end)
    it('should handle windows path', function()
      assert.are.equal(IO.dirname('C:\\a\\b'), 'C:/a')
    end)
  end)
  ...
  describe('.join', function()
    ...
    it('should handle root base', function()
      assert.are.equal(IO.join('/', 'a'), '/a')
    end)
  ...
  describe('.normalize', function()
    it('should expand tilde with slash', function()
      local home = assert(vim.uv.os_homedir())
      assert.are.equal(IO.normalize('~/a'), IO.normalize(home .. '/a'))
    end)
    it('should keep tilde without slash', function()
      assert.are.equal(IO.normalize('~user/a'), IO.join(IO.cwd(), '~user/a'))
    end)
  end)

  describe('.is_absolute', function()
    it('should handle', function()
      assert.is_true(IO.is_absolute('C:\\a\\b'))
      assert.is_true(IO.is_absolute('/path/to/dir'))
    end)
  end)
end)
